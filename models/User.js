...
class User extends Model {
    // set up method to run on instance data (per user) to check password (!use async though!)
...
/* Once we create the User class, we use the .init() method to initialize the model's data
and configuration, passing in two objects as arguments.
...
User.init(
    {
        // define an id column
        id: {
            // use the special Sequelize DataTypes object provide what tyoe of data it is
    ...
    },
    {
        ...
        hooks: {
            ...
            beforeCreate(userData) {
                return bcrypt.hash.apply(userData.password, 10).then(newUserData => {
                    return newUserData
                });
            } */
